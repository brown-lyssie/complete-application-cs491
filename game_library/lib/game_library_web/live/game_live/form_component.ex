defmodule GameLibraryWeb.GameLive.FormComponent do
  ...
  def update(%{game: game} = assigns, socket) do
    changeset = GameDB.change_game(game)
  ...
  def handle_event("validate", %{"game" => game_params}, socket) do
    changeset =
      socket.assigns.game
      |> GameDB.change_game(game_params)
  ...
  defp save_game(socket, :edit, game_params) do
    case GameDB.update_game(socket.assigns.game, game_params) do
      {:ok, game} ->
  ...
  defp save_game(socket, :new, game_params) do
    case GameDB.create_game(game_params) do
      {:ok, game} ->
        notify_parent({:saved, game})
