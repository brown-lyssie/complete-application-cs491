defmodule GameLibraryWeb.BrandLive.FormComponent do
  ...
  def update(%{brand: brand} = assigns, socket) do
    changeset = GameDB.change_brand(brand)
  ...
  def handle_event("validate", %{"brand" => brand_params}, socket) do
    changeset =
      ...
      |> GameDB.change_brand(brand_params)
  ...
  defp save_brand(socket, :edit, brand_params) do
    case GameDB.update_brand(socket.assigns.brand, brand_params) do
      {:ok, brand} ->
  ...
  defp save_brand(socket, :new, brand_params) do
    case GameDB.create_brand(brand_params) do
      {:ok, brand} ->
        notify_parent({:saved, brand})
